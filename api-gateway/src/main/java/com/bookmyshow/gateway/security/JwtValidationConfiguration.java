...
public class JwtValidationConfiguration {
    @Bean
    ReactiveJwtDecoder jwtDecoder(@Value("${security.jwt.public-key-location}") Resource publicKey,
        ...
        try (var input = publicKey.getInputStream()) {
            decoder = NimbusReactiveJwtDecoder.withPublicKey(RsaKeyConverters.x509().convert(input))
